...
func NewDataSource(ctx context.Context, dsn string) (*DataSource, error) {
	if strings.HasPrefix(dsn, "clickhouse://") {
		return newClickHouseDataSource(ctx, dsn)
	} else {
		return nil, fmt.Errorf("unsupported data source %q", dsn)
	}
}
