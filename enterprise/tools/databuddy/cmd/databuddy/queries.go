...
func SaveQuery(ctx context.Context, db QueryDB, store interfaces.Blobstore, queryID, query string) error {
	...
	q.Author = AuthenticatedUserEmail(ctx)
...
func ResolveMacros(ctx context.Context, db QueryDB, store interfaces.Blobstore, queryContent string) (string, error) {
	...
	for macro := range macroSet {
		...
		eg.Go(func() error {
			...
			q, err := db.GetQueryByName(ctx, name)
			if err != nil {
				if IsNotFound(err) {
					return NotFound(fmt.Errorf("resolve macro %q: query with name %q not found", macro, name))
				}
				return err
			}
